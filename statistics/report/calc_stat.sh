...
jq ' .authors + .editors + .contributors  | unique ' ${IBASE}/stakeholders.json > ${OUTPUT}.org0
jq ' [.[] | .aff = .affiliation.affiliationName| .name = .firstName + .lastName ]' ${OUTPUT}.org0 > $OUTPUT.org2
jq ' [ .[] | {"affiliation" : .aff |ascii_downcase , "name" : .name | ascii_downcase } ] ' ${OUTPUT}.org2 > ${OUTPUT}.org3
sed  "s/\s//g" ${OUTPUT}.org3 > ${OUTPUT}.org
